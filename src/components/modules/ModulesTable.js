...
const ModulesTable = ({ modulesItems, idCourse, getCourses }) => {
  ...
  const handleDelete = async (id) => {
    const response = await CoursesApiModel.deleteModule(id);
    switch (response.status) {
      case 201:
        getCourses();
        Swal.fire("¡Eliminado!", "Su modulo ha sido eliminado.", "success");
        break;
      default:
        Swal.fire({
          icon: "error",
